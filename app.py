...
def ensure_excel_exists():
    # Eğer dosya varsa ve içindeki tablo bozulduysa temiz bir başlangıç yapalım
    if os.path.exists(file_path):
        try:
            df_test = pd.read_excel(file_path, sheet_name="Arıza Takip Listesi", skiprows=16)
            df_test = df_test.dropna(subset=["Sıra"])
            if df_test.empty:
                os.remove(file_path)  # Boşsa dosyayı yenile
        except:
            pass
    ...
    if not os.path.exists(file_path):
        ...
        ws.cell(row=1, column=2, value="YALÇIN MARKET - DETAYLI TEKNİK ARIZA VE MÜDAHALE TAKİP FORMU")
        ...
        # Başlangıç örnek verileri ekleyelim ki tablo hemen dolu gelsin
        ornekler = [
            [None, 1, "2026-07-29 08:30", "Merkez Şube", "Şarküteri reyonu soğutma derecesi yükseliyor", "Soğutma / Soğuk Zincir", "Kritik", "Tamamlandı", "Ali Usta", "Zamanında", "Kompresör gazı yenilendi, test edildi.", "Onaylandı"],
            [None, 2, "2026-07-29 09:15", "Şube 02 - Bahçelievler", "Kasa 2 barkod okuyucu temassızlık", "Kasa & IT Donanım", "Normal", "Devam Ediyor", "Caner Bey", "Devam Ediyor", "Kablo değişimi yapılacak.", "Bekliyor"],
            [None, 3, "2026-07-29 10:00", "Merkez Şube", "Depo koridor aydınlatma armatürü arızalı", "Elektrik & Aydınlatma", "Düşük", "Tamamlandı", "Ahmet Usta", "Zamanında", "LED ampul değiştirildi.", "Onaylandı"]
        ]
...
def load_data():
    try:
        ...
        # Sıra sütununu sayıya çevirip filtreleyelim
        df = df[df["Sıra"].apply(lambda x: str(x).isdigit())]
        return df
    except Exception:
        return pd.DataFrame(columns=[
            "Sıra", "Bildirim Tarih/Saat", "Şube Adı", "Sorun / Arıza Açıklaması", 
            "Kategori", "Öncelik", "Durum", "Atanan Personel", "SLA Durumu", 
            "Çözüm Süresi / Açıklama", "Yön. Onay"
        ])
...
with col2:
    devam_eden = len(df_ariza[df_ariza["Durum"] == "Devam Ediyor"]) if not df_ariza.empty else 0
    st.metric("Devam Eden (Açık)", devam_eden)
with col3:
    kritik = len(df_ariza[df_ariza["Öncelik"] == "Kritik"]) if not df_ariza.empty else 0
    st.metric("Kritik Arızalar", kritik)
with col4:
    geciken = len(df_ariza[df_ariza["SLA Durumu"] == "Gecikti"]) if not df_ariza.empty else 0
...
if not df_ariza.empty and "Şube Adı" in df_ariza.columns:
    sube_filtre = st.selectbox("Şube Seçin", ["Tümü"] + list(df_ariza["Şube Adı"].dropna().unique()))
    if sube_filtre != "Tümü":
    ...
    st.dataframe(
        df_goster[[
            "Sıra", "Bildirim Tarih/Saat", "Şube Adı", "Sorun / Arıza Açıklaması", 
            "Kategori", "Öncelik", "Durum", "Atanan Personel", "SLA Durumu", "Çözüm Süresi / Açıklama"
        ]],
    ...
    with st.form("guncelleme_formu"):
        ...
        with col_g1:
            secilen_sira = st.selectbox("İşlem Yapılacak Arıza (Sıra No)", df_ariza["Sıra"].astype(str).tolist())
        with col_g2:
            yeni_durum = st.selectbox("Yeni Durum", ["Devam Ediyor", "Tamamlandı", "İptal Edildi"])
        with col_g3:
            atanan_personel = st.text_input("Atanan Teknik Personel", value="Ali Usta")
            
        cozum_aciklamasi = st.text_area("Çözüm Açıklaması / Yapılan İşlem", placeholder="Örn: Parça değiştirildi, test edildi.")
        ...
        if guncelle_submit:
            try:
                ...
                for row in range(18, ws.max_row + 1):
                    ...
                    if sira_hucre is not None and str(sira_hucre) == str(secilen_sira):
                ...
                if bulundu:
                    wb.save(file_path)
                    st.success(f"#{secilen_sira} numaralı arıza başarıyla güncellendi!")
                    st.cache_data.clear()
                    st.rerun()
                else:
    ...
    st.info("Henüz arıza kaydı bulunmuyor. Sol menüden ilk arıza kaydınızı oluşturabilirsiniz.")

# --- YENİ ARIZA KAYDI FORMU (SİDEBAR) ---
st.sidebar.header("➕ Yeni Arıza Bildirimi")
with st.sidebar.form("ariza_form"):
    yeni_sube = st.selectbox("Şube Adı", ["Merkez Şube", "Şube 02 - Bahçelievler", "Şube 03 - Meydan", "Şube 05 - Çarşı"])
    yeni_kategori = st.selectbox("Kategori", [
        "Soğutma / Soğuk Zincir", "Kasa & IT Donanım", 
        "Elektrik & Aydınlatma", "HVAC (Klima / Havalandırma)", "Mekanik & Raf / Kapı"
    ])
    ...
    if submit:
        if not yeni_aciklama.strip():
            ...
            try:
                ...
                wb.save(file_path)
                st.sidebar.success("Arıza kaydı Excel dosyasına başarıyla eklendi!")
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
